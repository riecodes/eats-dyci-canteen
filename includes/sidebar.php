    ...
    <ul class="nav nav-pills flex-column mb-auto">
        ...
        <?php elseif ($role === 'seller'): ?>
            ...
            <li><a href="index.php?page=seller_orders" class="nav-link <?php echo ($_GET['page'] ?? '') === 'seller_orders' ? 'active' : ''; ?>">
                ...
                if ($user_id) {
                    $seller_id = $user_id;
                    $unseen_orders = $pdo->prepare('SELECT COUNT(DISTINCT o.orderRef) FROM orders o JOIN order_items oi ON o.orderRef = oi.order_id JOIN products p ON oi.product_id = p.id JOIN stalls s ON p.stall_id = s.id WHERE s.seller_id = ? AND o.seen_by_seller = 0');
